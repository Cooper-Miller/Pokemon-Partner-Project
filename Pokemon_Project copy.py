...
class Pokemon():
    ...
    def Battle(self, opponent):
        ...
        if st.session_state.player.HP <= 0:
            ...
            if opponent.name in [pokemon.name for pokemon in st.session_state.yr_pokedex.pokemons]:
                if self.capture(st.session_state.yr_pokedex):
            ...
            del st.session_state.battle_log
            del st.session_state.turn_counter
            self.EXP += int(opponent.stat_total)/10
            return 1
    ...
    def Calculate_damage(self, index, opponent):
        ...
        for i in self.type_:
            if i in MOVES_DICTIONARY.get(opponent.moves[index])['super effective against']:
                ...
                eff *= 0.5
        mod = eff*rd*crit
        damage = int(((((opponent.level*2)/5+2)*MOVES_DICTIONARY.get(opponent.moves[index])['power']*(opponent.attack/self.defense))/50)*mod)
        self.HP -= damage
        return damage
    ...
    if len(st.session_state.yr_pokedex.pokemons) > 1:
        if 'chose_pokemon' not in st.session_state:
            with st.form(key=f'pokemon_choice_form_{st.session_state.form_counter}'):
                ...
                submitted = st.form_submit_button("Enter")
                if submitted:
                    if choice.title() in pokemons_list:
